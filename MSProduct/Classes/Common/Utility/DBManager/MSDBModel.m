...
@implementation MSDBModel

#pragma mark - override method
+ (void)initialize {
    if (self != [MSDBModel self]) {
        [self createTable];
    }
}
...
+ (BOOL)isExistInTable {
    __block BOOL res = NO;
    MSDBManager *PPDB = [MSDBManager shareInstance];
    [PPDB.dbQueue inDatabase:^(FMDatabase *db) {
...
+ (BOOL)createTable
{
    ...
    MSDBManager *PPDB = [MSDBManager shareInstance];
    [PPDB.dbQueue inTransaction:^(FMDatabase *db, BOOL *rollback) {
        NSString *tableName = NSStringFromClass(self.class);
...
//+ (BOOL)createTable
//{
//    FMDatabase *db = [FMDatabase databaseWithPath:[MSDBManager dbPath]];
//    if (![db open]) {
//        NSLog(@"数据库打开失败!");
//        return NO;
//    }
...
- (BOOL)saveOrUpdate {
    ...
    if ([primaryValue stringValue].length == 0) {
        return [self save];
    }
...
+ (BOOL)saveObjects:(NSArray *)array {
    //判断是否是JKBaseModel的子类
    for (MSDBModel *model in array) {
        if (![model isKindOfClass:[MSDBModel class]]) {
            return NO;
        }
    }
...
+ (NSArray *)findAll {
    ...
    [PPDB.dbQueue inDatabase:^(FMDatabase *db) {
        ...
        while ([resultSet next]) {
            MSDBModel *model = [[self.class alloc] init];
            for (int i=0; i< model.columeNames.count; i++) {
                NSString *columeName = [model.columeNames objectAtIndex:i];
                NSString *columeType = [model.columeTypes objectAtIndex:i];
               
                if ([columeType isEqualToString:SQLTEXT] || [columeType isEqualToString:@"TEXT primary key"]) {
